...
export async function POST(request: NextRequest) {
	try {
		...
		try {
			...
			const updatedCompletedAchievementIds = {
				...gameData.completedAchievementIds,
			}
			delete updatedCompletedAchievementIds[achievementId]

			const updatedGames = {
				...userGamesData.games,
				[gameId]: {
					...gameData,
					completedAchievementIds: updatedCompletedAchievementIds,
				},
			}
			...
			return NextResponse.json(
				{ message: 'Achievement marked as incomplete successfully' },
				{ status: 200 },
			)
		} finally {
			await client.close()
		...
		console.error('Error setting achievement incomplete:', error)
		return NextResponse.json(
